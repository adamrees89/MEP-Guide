...
New content should be added as a new chapter file inside `chapters/` and included from `MEP_Guide.tex`.
...
- Add the new chapter to `MEP_Guide.tex` in the appropriate `\part{}` section using an `\input{./chapters/yourfile.tex}` line.
...
3. Add the file to `MEP_Guide.tex` in the proper section.
