...
This is not likely to be a practical method of comfort cooling within the United Kingdom given the relatively humid weather and the need to provide outdoor air to spaces to allow occupants to occupy them safely. There will be specialist use cases for this type of cooler such as data centres, storage facilities, etc were high volumes of outdoor are not required; however, these types of facilities typically utilise closed loop refrigerant based cooling systems with specialised indoor cooling units such as \GLS{CRAH} units.
...
Using~\autoref{eq:evap_cooling_calculation_rearanged}, the return air is passed through an 85\% efficient evaporative cooler and it's temperature is reduced to \SI{16}{\celsius} as shown in~\autoref{eq:EEAHU_Example}. The \GLS{RH} of the return air stream is increased to 95\%.

\begin{equation}\label{eq:EEAHU_Example}
\Theta_{B} = 22 - 0.85(22 - 15) = 16.05
\end{equation}
...
The corresponding supply air temperature after the heat exchanger (Assumed 73\% efficient) would be \SI{19}{\celsius}, as shown in~\autoref{eq:EEAHU_PHX_Example}. Using the psychrometric chart, the supply air to the space would be \SI{19}{\celsius} dry-bulb and \SI{15.8}{\celsius} wet-bulb, which is a reduced humidity level when compared to direct AHU evaporative cooling and is approximately 80-85\% \GLS{RH}.

\begin{equation}\label{eq:EEAHU_PHX_Example}
t_2 = (0.73 \times (16 - 26.2)) + 26.2 = 18.75\si{\celsius}, say \SI{19}{\celsius}
\end{equation}